...
Saddle은 표준 Laravel 정책을 사용합니다. 모델에 정책을 등록하면 패널은 인덱스, 폼, 행 액션, 관계 피커 등 모든 곳에서 이를 강제합니다. Saddle은 기본적으로 실패 시 닫힘입니다. 등록된 정책이 없으면 모든 능력이 거부되므로, 접근 권한을 부여하려면 각 리소스의 모델에 정책을 등록하세요. 역할은 애플리케이션에 남습니다. 정책을 뒷받침하는 역할 패키지나 자체 레이어는 변경 없이 작동합니다.

### 패널 잠그기

등록된 정책이 없는 리소스는 기본적으로 접근할 수 없습니다(실패 시 닫힘). 따라서 누락된 정책이 조용히 데이터를 노출할 수 없습니다. 정책이 없는 리소스가 모든 인증된 사용자를 허용하는 실패 시 열림 관례를 선택하려면 `saddle.authorization.require_policy`를 `false`로 설정하세요. 어떤 패널 라우트가 실행되기 전에 전체 검사를 수행하도록 `saddle.middleware`에 gate middleware를 추가할 수도 있습니다. web guard가 최종 사용자와 관리자 사이에서 공유된다면 기본 실패 시 닫힘 자세를 유지하세요.
