...
Saddle は標準の Laravel ポリシーを使用します。モデルにポリシーを登録すると、パネルはインデックス、フォーム、行アクション、リレーションピッカーのすべてでそれを強制します。Saddle はデフォルトでフェイルクローズです。ポリシーが登録されていない場合、すべての能力は拒否されるため、アクセスを許可するには各リソースのモデルにポリシーを登録してください。ロールはアプリケーション内に残ります。ポリシーを支えるロールパッケージや自作レイヤーはそのまま動作します。

### パネルをロックダウンする

登録済みポリシーのないリソースは、デフォルトでアクセス不可です (フェイルクローズ)。そのため、ポリシーの登録漏れによってデータが密かに公開されることはありません。ポリシーのないリソースがすべての認証済みユーザーを許可するフェイルオープンの慣例を選ぶには、`saddle.authorization.require_policy` を `false` に設定します。任意のパネルルートが実行される前に包括的なチェックを行うため、gate middleware を `saddle.middleware` に追加することもできます。web guard をエンドユーザーと管理者で共有している場合は、デフォルトのフェイルクローズ姿勢を維持してください。
