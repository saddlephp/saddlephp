...
Saddle использует стандартные политики Laravel. Зарегистрируйте политику для модели, и панель будет применять ее везде: индекс, формы, действия строк и pickers отношений. Saddle по умолчанию работает по принципу fail-closed: если политика не зарегистрирована, каждая способность запрещена, поэтому зарегистрируйте политику для модели каждого ресурса, чтобы предоставить доступ. Роли остаются в вашем приложении: любой пакет ролей или самописный слой, на который опираются ваши политики, работает без изменений.

### Закрыть панель

Ресурсы без зарегистрированной политики по умолчанию недоступны (fail-closed), поэтому пропущенная политика никогда не сможет незаметно открыть данные. Чтобы выбрать соглашение fail-open, при котором ресурс без политики разрешает доступ каждому аутентифицированному пользователю, установите `saddle.authorization.require_policy` в `false`. Можно также добавить gate middleware в `saddle.middleware` для общей проверки перед запуском любого маршрута панели. Если ваш web guard общий для конечных пользователей и администраторов, сохраняйте стандартную позицию fail-closed.
