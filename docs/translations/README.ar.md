...
يستخدم Saddle سياسات Laravel القياسية. سجّل سياسة لنموذج، وستفرضها اللوحة في كل مكان: الفهرس، والنماذج، وإجراءات الصفوف، ومنتقيات العلاقات. يتبع Saddle الفشل المغلق افتراضياً: من دون سياسة مسجلة، تُرفض كل قدرة، لذلك سجّل سياسة لنموذج كل مورد لمنح الوصول. تبقى الأدوار في تطبيقك: أي حزمة أدوار أو طبقة محلية تدعم سياساتك تعمل من دون تغيير.

### إغلاق اللوحة

الموارد التي لا تملك سياسة مسجلة غير قابلة للوصول افتراضياً (فشل مغلق)، لذلك لا يمكن لسياسة منسية أن تكشف البيانات بصمت. لاختيار نمط الفتح عند الفشل، حيث يسمح المورد الذي لا يملك سياسة لكل مستخدم موثق، اضبط `saddle.authorization.require_policy` على `false`. يمكنك أيضاً إضافة middleware gate إلى `saddle.middleware` لإجراء فحص شامل قبل تشغيل أي مسار للوحة. إذا كان web guard لديك مشتركاً بين المستخدمين النهائيين والمديرين، فحافظ على وضع الفشل المغلق الافتراضي.
