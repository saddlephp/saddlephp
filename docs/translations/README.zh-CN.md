...
Saddle 使用标准 Laravel 策略。为模型注册策略后，面板会在所有地方强制执行: 索引、表单、行操作和关系选择器。Saddle 默认采用关闭式失败: 没有注册策略时，所有能力都会被拒绝，因此请为每个资源的模型注册策略以授予访问权限。角色保留在你的应用中: 任何支持你策略的角色包或自建层都无需更改即可工作。

### 锁定面板

没有注册策略的资源默认不可访问(关闭式失败)，因此遗忘的策略永远不会悄然暴露数据。若要选择开放式失败约定，即没有策略的资源允许所有认证用户，请将 `saddle.authorization.require_policy` 设为 `false`。你也可以向 `saddle.middleware` 添加 gate middleware，在任何面板路由运行前做统一检查。如果你的 web guard 在终端用户和管理员之间共享，请保持默认的关闭式失败姿态。
