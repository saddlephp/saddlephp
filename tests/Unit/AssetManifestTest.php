...
{
    File::ensureDirectoryExists(public_path('vendor/rodeo'));
    File::put(public_path('vendor/rodeo/manifest.json'), json_encode($manifest));
}
...
it('resolves script and style urls from the manifest entry', function () {
    ...
    expect(AssetManifest::script())->toEndWith('vendor/rodeo/assets/app-abc123.js')
        ->and(AssetManifest::styles())->toHaveCount(1)
        ->and(AssetManifest::styles()[0])->toEndWith('vendor/rodeo/assets/app-def456.css')
...
it('handles a manifest without css gracefully', function () {
    ...
    expect(AssetManifest::styles())->toBe([])
        ->and(AssetManifest::script())->toEndWith('vendor/rodeo/assets/app-abc123.js');
});

it('returns null script for malformed manifest json', function () {
    File::ensureDirectoryExists(public_path('vendor/rodeo'));
    File::put(public_path('vendor/rodeo/manifest.json'), '{not-json');
